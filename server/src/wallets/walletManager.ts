...
const ASSOCIATED_TOKEN_PROGRAM = new PublicKey('ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJe1bR');
...
function decrypt(encryptedKey: string, iv: string): Uint8Array {
  const decipher  = crypto.createDecipheriv('aes-256-cbc', getEncKey(), Buffer.from(iv, 'hex'));
...
export class WalletManager {
  ...
  async withdrawSOL(
    ...
    if (!confirm.confirmed) {
      ...
      pendingTxRepo.insert(txHash, userId, null, lamports, 'So11111111111111111111111111111111111111112', destination);
  ...
  async withdrawToken(
    ...
    const mintInfo = await this.connection.getParsedAccountInfo(mintPubkey, 'confirmed');
    ...
    if (!confirm.confirmed) {
      pendingTxRepo.insert(txHash, userId, null, Number(rawAmount), tokenMint, destination);
      return { txHash, status: 'PENDING' };
    }
  ...
  private async confirmWithTimeout(txHash: string): Promise<{ confirmed: boolean; err?: string }> {
