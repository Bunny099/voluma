...
type View = 'feed' | 'triggers' | 'conditions' | 'wallet' | 'history';
...
const NAV_ITEMS: { view: View; label: string; short: string; icon: React.ReactNode }[] = [
  {
    ...
    icon: <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><circle cx="7.5" cy="7.5" r="2.5" stroke="currentColor" strokeWidth="1.4"/><circle cx="7.5" cy="7.5" r="5.5" stroke="currentColor" strokeWidth="1" opacity="0.4"/><circle cx="7.5" cy="7.5" r="0.8" fill="currentColor"/></svg>,
  },
  {
    view: 'triggers', label: 'Executions', short: 'Runs',
    icon: <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><path d="M8 2L4.5 8H7L6 13L10.5 7H8L8 2Z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/></svg>,
  },
  {
    view: 'conditions', label: 'Automations', short: 'Flows',
    icon: <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><rect x="1.5" y="3" width="4" height="4" rx="1" stroke="currentColor" strokeWidth="1.4"/><rect x="9.5" y="8" width="4" height="4" rx="1" stroke="currentColor" strokeWidth="1.4"/><path d="M5.5 5H9M9 5V8.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>,
  },
  {
    view: 'wallet', label: 'Wallet', short: 'Wallet',
    icon: <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><rect x="1" y="3.5" width="13" height="8.5" rx="1.5" stroke="currentColor" strokeWidth="1.4"/><path d="M1 6.5H14" stroke="currentColor" strokeWidth="1.4"/><circle cx="11" cy="9.5" r="1" fill="currentColor"/></svg>,
  },
  {
    view: 'history', label: 'History', short: 'Hist',
    icon: <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><rect x="1.5" y="1.5" width="12" height="12" rx="2" stroke="currentColor" strokeWidth="1.4"/><path d="M4 7.5L6.5 10L11 5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>,
  },
];

interface ToastProps {
  id: string; kind: 'success' | 'error' | 'pending';
  message: string; txHash?: string; onDismiss: (id: string) => void;
}
...
function TradeToast({ id, kind, message, txHash, onDismiss }: ToastProps) {
  ...
  const label     = isOk ? 'Confirmed' : isPending ? 'Pending' : 'Failed';
  return (
    <div style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'10px 14px', borderRadius:10, background:bg, border:`1px solid ${border}`, boxShadow:'0 4px 20px rgba(0,0,0,0.5)', animation:'toast-in 0.25s ease-out both', minWidth:260, maxWidth:340 }}>
      <div style={{ width:20, height:20, borderRadius:'50%', flexShrink:0, marginTop:1, background:`${color}22`, display:'flex', alignItems:'center', justifyContent:'center', color, fontSize:11 }}>{icon}</div>
      <div style={{ flex:1, minWidth:0 }}>
        <p style={{ fontSize:'0.78rem', fontWeight:600, color, marginBottom: txHash ? 3 : 0 }}>{label}</p>
        <p style={{ fontFamily:'JetBrains Mono, monospace', fontSize:'0.66rem', color:`${color}99`, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{message}</p>
        {txHash && (
      ...
      <button onClick={() => onDismiss(id)} style={{ background:'none', border:'none', cursor:'pointer', color:'#5a6b7e', fontSize:14, padding:2, lineHeight:1 }}>×</button>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const {
    ...
    tradeToasts, dismissToast, pendingTxs, systemStatus, _onTradeSuccessRef,
  } = useSocket(userId);

  const [view, setView] = useState<View>('feed');
  const latestTriggerKey = triggers[0] ? `${triggers[0].conditionId}:${triggers[0].matchedAt}` : null;
  const rpcLabel = systemStatus?.rpc.activeProvider.label ?? 'Solana Mainnet';
  const rpcState = systemStatus?.rpc.healthState ?? 'HEALTHY';
  ...
  if (sessionLoading || !sessionData) {
    return (
      <div style={{ height:'100vh', background:'#070b10', display:'flex', alignItems:'center', justifyContent:'center' }}>
        <style>{`@keyframes dot-pulse{0%,100%{opacity:1}50%{opacity:0.25}}`}</style>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:6, height:6, borderRadius:'50%', background:'#d4ff00', animation:'dot-pulse 1.5s infinite' }} />
          <span style={{ fontFamily:'JetBrains Mono,monospace', fontSize:12, color:'#5a6b7e', letterSpacing:'0.06em' }}>
            {sessionLoading ? 'AUTHENTICATING…' : 'REDIRECTING…'}
          </span>
        </div>
      </div>
    );
    ...
    <div className="vdb-root">
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        .vdb-root { height:100vh; background:#070b10; color:#e8ecf0; font-family:'DM Sans',system-ui,sans-serif; display:flex; flex-direction:column; overflow:hidden; -webkit-font-smoothing:antialiased; }
        .vdb-topbar { height:52px; flex-shrink:0; display:flex; align-items:center; justify-content:space-between; padding:0 1.25rem; border-bottom:1px solid rgba(255,255,255,0.07); background:rgba(7,11,16,0.97); backdrop-filter:blur(12px); position:relative; z-index:50; }
        ...
        .vdb-tab { display:flex; align-items:center; gap:7px; padding:6px 14px; border-radius:7px; font-size:0.78rem; font-weight:500; letter-spacing:0.01em; cursor:pointer; border:1px solid transparent; outline:none; background:transparent; color:#4a5260; transition:all 0.15s; position:relative; }
        ...
        .vdb-pipeline-label { font-family:'JetBrains Mono',monospace; font-size:0.58rem; color:#506070; letter-spacing:0.1em; text-transform:uppercase; margin-bottom:10px; }
        .vdb-pipeline-node { display:flex; align-items:center; gap:8px; padding:6px 8px; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.06); border-radius:7px; margin-bottom:2px; }
        ...
        .vdb-pipeline-node-text { font-size:0.68rem; font-weight:500; color:#5a6b7e; flex:1; }
        .vdb-pipeline-node.live .vdb-pipeline-node-text { color:#8a939f; }
        .vdb-pipeline-connector { display:flex; align-items:center; gap:4px; padding:1px 8px 1px 19px; color:rgba(212,255,0,0.2); font-family:'JetBrains Mono',monospace; font-size:0.6rem; }
        ...
        .vdb-network-label { font-family:'JetBrains Mono',monospace; font-size:0.62rem; color:#5a6b7e; letter-spacing:0.04em; }
        .vdb-userid { font-family:'JetBrains Mono',monospace; font-size:0.58rem; color:#4a5a6e; margin-top:2px; padding-left:10px; display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
        ...
        .vdb-mobile-item { flex:1; display:flex; flex-direction:column; align-items:center; gap:3px; padding:10px 4px; cursor:pointer; background:transparent; border:none; outline:none; color:#5a6b7e; transition:color 0.15s; position:relative; }
        ...
        .vdb-signout-btn:hover { color:#8a939f !important; border-color:rgba(255,255,255,0.14) !important; }
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <button key={item.view} className={`vdb-tab${view === item.view ? ' active' : ''}`} onClick={() => setView(item.view)}>
                ...
                {count !== undefined && count > 0 && <span className="vdb-tab-badge">{count > 999 ? '999+' : count}</span>}
              </button>
            );
          })}
        </div>

        {/* Right: connection status + user info */}
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div className="vdb-status">
            ...
            <span className="vdb-status-label" style={{ color: connected ? '#d4ff00' : '#f87171' }}>
              {connected ? (rpcState === 'FALLBACK' ? 'FALLBACK' : rpcState === 'DEGRADED' ? 'DEGRADED' : 'LIVE') : 'OFFLINE'}
            </span>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, paddingLeft:10, borderLeft:'1px solid rgba(255,255,255,0.06)' }}>
            {sessionData.user.image && (
              <img src={sessionData.user.image} alt="" referrerPolicy="no-referrer"
                style={{ width:22, height:22, borderRadius:'50%', border:'1px solid rgba(255,255,255,0.08)', flexShrink:0 }} />
            )}
            <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:'0.58rem', color:'#5a6b7e', letterSpacing:'0.04em', maxWidth:120, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
              {sessionData.user.email}
            </span>
            <button className="vdb-signout-btn" onClick={handleSignOut}
              style={{ fontFamily:'JetBrains Mono, monospace', fontSize:'0.58rem', color:'#5a6b7e', background:'transparent', border:'1px solid rgba(255,255,255,0.07)', borderRadius:5, padding:'3px 8px', cursor:'pointer', letterSpacing:'0.04em', transition:'color 0.15s, border-color 0.15s', flexShrink:0 }}>
              sign out
            </button>
          </div>
        </div>
      ...
      <div className="vdb-body">
            ...
            {NAV_ITEMS.map(item => {
              ...
              return (
                <button key={item.view} className={`vdb-sidebar-item${view === item.view ? ' active' : ''}`} onClick={() => setView(item.view)}>
                  {item.icon}{item.label}
                  {count !== undefined && count > 0 && <span className="vdb-sidebar-badge">{count > 999 ? '999+' : count}</span>}
                </button>
              );
            })}
          </nav>
          ...
          <div className="vdb-network">
            <div className="vdb-network-inner">
              <div className="vdb-network-row">
                ...
                <span className="vdb-network-label">{rpcLabel}</span>
              </div>
              {systemStatus && (
                <span className="vdb-userid" title={systemStatus.rpc.activeProvider.label}>
                  {systemStatus.rpc.healthState.toLowerCase()} · reconnects {systemStatus.rpc.activeProvider.reconnectCount}
                </span>
              )}
        ...
        <main className="vdb-main">
          <div key={view} className="vdb-view">
            ...
            {view === 'history'    && <TradeHistory userId={userId} />}
          </div>

          {tradeToasts.length > 0 && (
            <div style={{ position:'absolute', bottom:20, right:20, display:'flex', flexDirection:'column', gap:8, zIndex:100, pointerEvents:'none' }}>
              {tradeToasts.map(t => (
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <button key={item.view} className={`vdb-mobile-item${view === item.view ? ' active' : ''}`} onClick={() => setView(item.view)}>
              ...
              {count !== undefined && count > 0 && <span className="vdb-mobile-badge">{count > 99 ? '99+' : count}</span>}
            </button>
          );
        })}
      </nav>
