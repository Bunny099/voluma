...
const LBL: React.CSSProperties = {
  ...
  textTransform: 'uppercase' as const, color: '#4a5260',
  fontFamily: 'JetBrains Mono, monospace', display: 'block', marginBottom: 6,
};
...
function actionPreview(action: ExecutionAction | undefined): string[] {
  ...
  switch (action.type) {
    case 'TRADE':
      ...
      if (action.tradeDirection === 'BUY' && action.tradeAmountSol)
        lines.push(`${action.tradeAmountSol} SOL`);
      if (action.tradeDirection === 'SELL' && action.tradeSellPercent)
        lines.push(`${action.tradeSellPercent}% of balance`);
      if (action.tradeTokenMint) lines.push(action.tradeTokenMint.slice(0,8)+'...');
      break;
    case 'WEBHOOK':
      if (action.webhookUrl) {
        try { lines.push(new URL(action.webhookUrl).hostname); } catch { lines.push('endpoint set'); }
      }
      break;
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', flexShrink: 0, gap: 0, padding: '0 4px' }}>
      <svg width="72" height="24" viewBox="0 0 72 24" fill="none" style={{ overflow: 'visible' }}>
        ...
        {active && (
          <circle r="3" fill={color} opacity="0.9" filter="url(#conn-glow)">
...
function NodeCard({ label, sublabel, icon, color, dim, border, preview, isSelected, isConfigured, nodeType, onClick }: NodeCardProps) {
  ...
  return (
    <div
      ...
      style={{
        flex: 1, minWidth: 0,
        background: isSelected ? dim : hovered ? 'rgba(255,255,255,0.025)' : 'rgba(255,255,255,0.015)',
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        {isConfigured && (
          <div style={{
            ...
            background: isSelected ? color : 'rgba(255,255,255,0.2)', transition: 'all 0.2s',
          }} />
        )}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: '0.68rem', color: '#3d4452', fontFamily: 'JetBrains Mono, monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sublabel}</div>
        </div>
      </div>
      {preview.length > 0 ? (
        ...
        <div style={{
          ...
          border: `1px dashed ${isActive ? `${color}30` : 'rgba(255,255,255,0.07)'}`,
          borderRadius: 8, padding: '6px 9px',
          fontFamily: 'JetBrains Mono, monospace', fontSize: '0.62rem', color: '#2e3540',
      ...
      {isSelected && (
        <div style={{
          ...
          borderBottom: `8px solid ${color}40`,
        }} />
      )}
    </div>
  );
...
function ConfigPanel({ children, color, label, visible }: {
  children: React.ReactNode; color: string; label: string; visible: boolean;
}) {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (visible && ref.current) {
      ref.current.style.maxHeight = ref.current.scrollHeight + 64 + 'px';
    } else if (ref.current) {
  ...
  return (
    <div ref={ref} style={{ maxHeight: 0, overflow: 'hidden', transition: 'max-height 0.35s cubic-bezier(0.4,0,0.2,1)' }}>
      <div style={{
        marginTop: 8, background: 'rgba(255,255,255,0.015)',
        border: `1px solid ${color}25`, borderRadius: 14, overflow: 'hidden',
      }}>
        <div style={{
          padding: '10px 16px', background: `linear-gradient(90deg, ${color}10, transparent)`,
          borderBottom: `1px solid ${color}18`, display: 'flex', alignItems: 'center', gap: 8,
        }}>
          ...
          <span style={{
            ...
            letterSpacing: '0.12em', color, textTransform: 'uppercase' as const,
...
export default function ConditionBuilder({ userId, onCreated }: Props) {
  ...
  const balanceInsufficient =
    ...
    tradeAmountSol > 0 &&
    walletState.balanceSol < requiredSol;
  ...
  useEffect(() => {
    if (!isTradeAction || !userId) { setWalletState(null); return; }
    let cancelled = false;
  ...
  const handleNodeClick = (panel: ActivePanel) => {
  ...
  const handleSubmit = async () => {
    ...
    if (isTradeAction) {
      ...
      if (isBuy) {
        if (!currentAction.tradeAmountSol || tradeAmountSol <= 0)
          { setError('Enter the SOL amount to spend'); setActivePanel('action'); return; }
      }
      if (isSell) {
        const pct = currentAction.tradeSellPercent;
        if (!pct || pct <= 0 || pct > 100)
          { setError('Enter a sell percentage between 1 and 100'); setActivePanel('action'); return; }
      }
    }
    setSaving(true);
    try {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      <style>{`
        ...
        .cb2-type-btn:hover { opacity: 0.85; }
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '12px 14px', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.1em', color: '#3d4452', textTransform: 'uppercase' as const, flexShrink: 0, whiteSpace: 'nowrap' }}>
            AUTOMATION
          </span>
          <input
            className="cb2-name"
            placeholder="e.g. BONK whale alert, USDC volume spike..."
            value={form.name ?? ''}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', color: '#2e3540', whiteSpace: 'nowrap' }}>cd</span>
            <input
              className="cb2-input"
              type="number" min={0}
            ...
            />
            <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', color: '#2e3540' }}>s</span>
          </div>
        </div>
      </div>
      ...
      <ConfigPanel color={triggerCfg.color} label={triggerCfg.label} visible={activePanel === 'trigger'}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <div>
                ...
                <input className="cb2-input" type="number" min={1} placeholder={form.type === 'SWAP_BURST' ? '50' : '1000'} value={form.type === 'SWAP_BURST' ? (form.minSwaps ?? '') : (form.minVolumeSol ?? '')} onChange={e => form.type === 'SWAP_BURST' ? set('minSwaps', e.target.value ? Number(e.target.value) : undefined) : set('minVolumeSol', e.target.value ? Number(e.target.value) : undefined)} style={INP} />
              </div>
              <div>
        ...
        {form.type === 'LARGE_TRANSFER' && (
          <div>
            ...
            <p style={{ fontSize: '0.68rem', color: '#3d4452', marginTop: 6, fontFamily: 'DM Sans, sans-serif' }}>
              Fires on any transfer exceeding this threshold on mainnet.
            </p>
          </div>
        )}
        ...
        <div style={{ marginTop: 14, display: 'flex', justifyContent: 'flex-end' }}>
          <button onClick={() => setActivePanel('action')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, background: `${triggerCfg.color}15`, border: `1px solid ${triggerCfg.color}35`, color: triggerCfg.color, fontSize: '0.75rem', fontWeight: 700, fontFamily: 'DM Sans, sans-serif', cursor: 'pointer', transition: 'all 0.15s' }}>
            Configure Action
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M3 6H9M9 6L6.5 3.5M9 6L6.5 8.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
          </button>
        </div>
      </ConfigPanel>
      ...
      <ConfigPanel color={actionCfg.color} label={actionCfg.label} visible={activePanel === 'action'}>
        ...
        {currentAction?.type === 'WEBHOOK' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <button onClick={handleTestHook} disabled={testingHook || !currentAction.webhookUrl} style={{ padding: '0 14px', borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#8a939f', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', flexShrink: 0, fontFamily: 'DM Sans,sans-serif', opacity: testingHook || !currentAction.webhookUrl ? 0.5 : 1, transition: 'all 0.15s' }}>
                {testingHook ? '...' : 'Test'}
              </button>
            </div>
            {hookResult && (
        ...
        {currentAction?.type === 'LOG' && (
          <div style={{ padding: '12px 14px', borderRadius: 10, background: 'rgba(107,114,128,0.06)', border: '1px solid rgba(107,114,128,0.2)' }}>
            <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem', color: '#5c6472' }}>
              Writes structured JSON to server stdout. No configuration needed.
            </p>
          </div>
        )}
        ...
        {currentAction?.type === 'TRADE' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {!walletChecking && walletState?.exists === true && (
              <div style={{
                background: (isBuy && balanceInsufficient) ? 'rgba(251,191,36,0.07)' : 'rgba(212,255,0,0.05)',
                border: `1px solid ${(isBuy && balanceInsufficient) ? 'rgba(251,191,36,0.2)' : 'rgba(212,255,0,0.18)'}`,
                borderRadius: 9, padding: '8px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: (isBuy && balanceInsufficient) ? '#fbbf24' : '#d4ff00' }}>
                  {(isBuy && balanceInsufficient) ? '⚠ Low SOL balance' : '✓ Wallet ready'}
                </span>
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem', color: '#c4ccd6' }}>
                  ◎ {(walletState.balanceSol ?? 0).toFixed(4)} SOL
                </span>
              </div>
            )}
                ...
                {(['BUY','SELL'] as const).map(dir => (
                  <button key={dir}
                    ...
                    style={{
                      ...
                      background: currentAction.tradeDirection === dir
                        ? dir === 'BUY' ? '#d4ff00' : '#f87171'
                        : 'rgba(255,255,255,0.04)',
                      color: currentAction.tradeDirection === dir
                        ? dir === 'BUY' ? '#070b10' : '#fff'
                        : '#4a5260',
                      boxShadow: currentAction.tradeDirection === dir && dir === 'BUY'
                        ? '0 4px 16px rgba(212,255,0,0.2)' : 'none',
                    }}>
                    {dir === 'BUY' ? '↑ BUY' : '↓ SELL'}
                  </button>
                ))}
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 5, marginBottom: 7 }}>
                {KNOWN_TOKENS.map(t => {
                  ...
                  return (
                    <button key={t.mint} className="cb2-tok-btn"
                      onClick={() => setTradeMint(t.mint)}
                ...
                {isSell ? (
                  <>
                    ...
                    <input
                      ...
                      placeholder="e.g. 50"
                      value={currentAction.tradeSellPercent ?? ''}
                      onChange={e => setAction({ tradeSellPercent: e.target.value ? Math.min(100, Math.max(1, Number(e.target.value))) : undefined })}
                      style={INP}
                    />
                    <p style={{ fontSize: '0.65rem', color: '#3d4452', marginTop: 4, fontFamily: 'JetBrains Mono, monospace' }}>
                      % of actual wallet token balance at execution time
                    </p>
                  </>
                ) : (
                  <>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 4, background: 'rgba(255,255,255,0.04)', padding: 4, borderRadius: 9, marginBottom: 8 }}>
                {([['once','Once'],['limited','Limited'],['unlimited','Unlimited']] as const).map(([mode,label]) => (
                  <button key={mode} className="cb2-seg-btn"
                    onClick={() => applyExecMode(mode as ExecMode)}
              ...
              {execMode === 'limited' && (
                <input className="cb2-input" type="number" min={1} placeholder="Max executions" value={form.maxExecutions ?? ''} onChange={e => set('maxExecutions', e.target.value ? Number(e.target.value) : undefined)} style={{ ...INP, marginBottom: 6 }} />
              )}
              <p style={{ fontSize: '0.68rem', color: '#3d4452' }}>
      ...
      <button
        className={saved ? 'cb2-success' : ''}
        onClick={handleSubmit}
        disabled={!canSubmit}
        style={{
          marginTop: 16, width: '100%', height: 46, borderRadius: 12, border: 'none',
          background: saved ? 'rgba(212,255,0,0.12)' : canSubmit ? '#d4ff00' : 'rgba(255,255,255,0.05)',
          color: saved ? '#d4ff00' : canSubmit ? '#070b10' : '#3d4452',
          fontSize: '0.85rem', fontWeight: 700, letterSpacing: '0.04em',
          fontFamily: 'DM Sans, sans-serif', cursor: canSubmit ? 'pointer' : 'not-allowed',
          transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          boxShadow: canSubmit && !saved ? '0 4px 24px rgba(212,255,0,0.2)' : 'none',
        }}
      >
        {saved ? (
          <><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7L5.5 10.5L12 3" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>Automation Live</>
        ) : saving ? (
          <><div style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid rgba(7,11,16,0.3)', borderTopColor: '#070b10', animation: 'spin 0.8s linear infinite' }} />Creating...</>
        ) : (
